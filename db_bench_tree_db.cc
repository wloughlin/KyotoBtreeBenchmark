...
static int queries, inserts, deletes;

// Number of key/values to place in database
static int FLAGS_num = 1000000;

// Number of read operations to do.  If negative, do FLAGS_num reads.
static int FLAGS_reads = 500000;
static int FLAGS_num_ops = 1000;
...
class Benchmark {
 private:
  ...
  void perfect_shuffle(int* buf, int len) {
    for (int i = 0; i < len; i++) {
      int r = std::rand() % (len - i);
      int posv = buf[i];
      buf[i] = buf[len-r-1];
      buf[len-r-1] = posv;
    }
  ...
  void Stop(const Slice& name) {
    ...
    fprintf(stdout, "%-12s : %11.3f milis;%s%s\n",
            name.ToString().c_str(),
            (finish - start_) * 1e3,
            (message_.empty() ? "" : " "),
            message_.c_str());
 ...
 private:
    ...
    if (FLAGS_b > 0) {
      db_->tune_page((FLAGS_key_size+FLAGS_value_size)*FLAGS_b);
    }
...
*/  
	...
	fflush(stdout);
      ...
      *(int *)key = i;
      *(int *)value = i + 1;
      if (!db_->set(key, FLAGS_key_size, value, FLAGS_value_size)) {
  ...
  void Write(bool sync, Order order, DBState state,
    ...
    char *key = (char *)calloc(1, FLAGS_key_size);
    char *value = (char *)calloc(1, FLAGS_value_size);
    int *keys = (int *)calloc(num_entries, sizeof(int));
    for (int i = 0; i < num_entries; i++) {
        keys[i] = i;
    }
    perfect_shuffle(keys, num_entries);
    Start();
    for (int i = 0; i < num_entries; i++)
    {
      *(int *)key = keys[i];
  ...
  void ReadRandom() {
    char *key = (char *)calloc(1, FLAGS_key_size);
    int *keys = (int *)calloc(reads_, sizeof(int));
    size_t val_size;
    for (int i = 0; i < reads_; i++) {
        keys[i] = i;
    }
    perfect_shuffle(keys, reads_);
    Start();
    for (int i = 0; i < reads_; i++)
    {
      *(int *)key = keys[i];
      db_->get(key, FLAGS_key_size, &val_size); 
    }
    Stop("randread");
    free(key);
    free(keys);
...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    int n;
      ...
      FLAGS_page_size = n;
    } else if (sscanf(argv[i], "--b_size=%d%c", &n, &junk) == 1) {
      FLAGS_b = n;
